...
$result_bill = mysqli_query($conn, $sql_bill_no);
	...
	if (mysqli_num_rows($result_bill) > 0) {
		...
		?>
		<form action="perchase_reject_confirm.php" method="post">
    ...
    <tbody>
      <tr>
	   <td><?php echo $row['id'];?></td>
        <td><input type="checkbox" name="check_list[]" value="<?php echo $row['id'];?>" checked style="display:none"> <td>
  ...
  <button class="btn btn-default btn-danger" name="submit"> Confirm To Reject</button>
</form>
  <?php
	}
