...
$sql="SELECT * FROM reports WHERE report_transection_status='cheque withdraw' and report_transection_date BETWEEN '$start_date' AND '$end_date'";
$result = mysqli_query($conn, $sql);

	if (mysqli_num_rows($result) > 0) {
		
	?>
<h4 class="text-center alert alert-success ">(Cash/Cheque) Transactions during this period !</h4>
...
<?php	
			...
			$timeStamp = date( "m/d/Y", strtotime($acc_transection_date));
			 ...
			 if($report_transection_status=='cash deposit'){
				 $report_bank_cash_or_cheque_deposit_amount=$row['report_bank_cash_or_cheque_deposit_amount'];
				 ...
				 <tr>
				   ...
				   <td><?php echo $report_bank_cash_or_cheque_deposit_amount;?></td>
				   <td><?php ?></td>
			...
			if($report_transection_status=='cash withdraw'){
...
<form action="report_cash_or_cheque_deposit_or_withdraw.php" method="post">
