...
<?php	
$sql_purchase_bill_no="SELECT * FROM perchases WHERE parchase_approve_status='pending' and parchase_approve_status!='approved' ORDER BY id DESC LIMIT 1";
			...
			if(isset($bill_no)){
				if($bill_no >50){
					$y=$bill_no - 20;
								for ($x = $bill_no; $x >= $y; $x--) {
									//echo $x."</br>";
									$sql_bill_no_serch="SELECT * FROM perchases WHERE parchase_bill_number=$x and parchase_approve_status='pending' and parchase_approve_status!='approved'";
								...
								<!--  if price quotation >10 -->
									<tr>
										...
										<td><a href="purchase_info.php?purchase_bill_no=<?php echo $x; ?>&supplier_name=<?php echo $supplier_name; ?>" >Approve</a></td>
				...
				<?php		
					...
					if(($bill_no <=50) and ($bill_no >=0) ){
						...
						for ($x = $bill_no; $x >= 1; $x--) {
							...
							$sql_bill_no_serch="SELECT * FROM perchases WHERE parchase_bill_number=$x and parchase_approve_status='pending'";
									$result_bill_no_serch = mysqli_query($conn, $sql_bill_no_serch);
									if (mysqli_num_rows($result_bill_no_serch) > 0) {
									$row_bill_no_serch = mysqli_fetch_array($result_bill_no_serch);
									$supplier_name=$row_bill_no_serch['parchase_supplier_full_name'];
